...
export default{
    ...
    mounted(){
        ...
        const map = new google.maps.Map(this.$refs["map"], {
            center:new google.maps.LatLng(40.691930, -73.984480),
            zoom: 15,
            mapTypeId:"roadmap"
        });
        ...
        EventBus.$on("routes-data", routes => {
            ...
            (response, status) =>{
                if(status === "OK"){
                    ...
                    const distanceDurationLable = new google.maps.InfoWindow({
                    ...
                    distanceDurationLable.open(this.map, null);

                    this.createPolylineWith(
                        [{lat: origin.lat, lng: origin.lng},{lat:overviewPath[0].lat(), lng:overviewPath[0].lng()}], 
                        color);
                    this.createPolylineWith(
                        [{lat: destination.lat, lng: destination.lng},{lat:overviewPath[overviewPath.length - 1].lat(), lng:overviewPath[overviewPath.length - 1].lng()}],
                        color);
